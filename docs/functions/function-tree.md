...
> 多租户 SaaS 身份管理 — React 19 前端。consumes `saas-identity-platform-shared` TypeSpec SSOT。
> 字段照抄 BASE（ADR-0024）；结构性对齐按 shared 9/7 重构。
...
| 子项 ID | 名称 | 类型 | 交付 | 说明 | 状态 |
|---|---|---|---|---|---|
| M01.F04.I03 | 密码登录 UI | 页面 | 仅前端 | saas-react LoginPage 登录页面 | 已上线 |
...
> OAuth 2.0 authorization_code + refresh_token（react 仓通过 OAuth client lib 调用 saas-nextjs IdP）。

| 子项 ID | 名称 | 类型 | 交付 | 说明 | 状态 |
| --- | --- | --- | --- | --- | --- |
| M04.F03.I01 | 授权码签发 | 接口 | 前端+后端 | react 仓 src/api/endpoints/oauth/oauth.ts oAuthAuthorize + src/lib/oauth-flow.ts.oauthAuthorize | 已上线 |
